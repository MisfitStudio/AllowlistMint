...
async function batchDevMint(wallet,address,qty,additionalIndex) {
    const tx = await wallet.devMint(address,qty);
    let receipt = await tx.wait()
    if (!additionalIndex) {
        additionalIndex = 0
    }
    let i = 0
    while (i < receipt.events.length) {

        expect(receipt.events[i].args[0]).to.equal(ethers.constants.AddressZero)
        expect(receipt.events[i].args[1]).to.equal(address)
        expect(receipt.events[i].args[2]).to.equal(ethers.BigNumber.from(i + additionalIndex))
        expect(receipt.events[i].args.from).to.equal(ethers.constants.AddressZero)
        expect(receipt.events[i].args.to).to.equal(address)
        expect(receipt.events[i].args.tokenId).to.equal(ethers.BigNumber.from(i + additionalIndex))

        i++
    }
}
async function batchMint(wallet,address,qty,additionalIndex) {
    const tx = await wallet.allowlistMint(qty,{value: minPrice.mul(qty)});
    let receipt = await tx.wait()
    if (!additionalIndex) {
        additionalIndex = 0
    }
...
describe("MisfitMinter", function () {
    ...
    it.only("devMint initial 200", async function () {
        await batchDevMint(misfit,owner.address,200)
    })
    it.only("devMint cannot mint more then 200", async function () {
        await expect(
            misfit.devMint(owner.address,300)
        ).to.be.revertedWith("too many already minted before dev mint");
        await batchDevMint(misfit,owner.address,200)
        await expect(
            misfit.devMint(owner.address,1)
        ).to.be.revertedWith("too many already minted before dev mint");

    })
    it.only("devMint cannot be less then 200", async function () {
        await expect(
            misfit.devMint(owner.address,100)
        ).to.be.revertedWith("can only mint a multiple of the maxBatchSize");


    })
    it.only("owner can seedAllowlist and numSlots", async function () {
        const allowList = [addr1.address,addr2.address];
        const numSlots = [3,3]
        console.log("allowList",allowList,numSlots)
        // expect(await misfit.allowlist()).to.equal([])
        expect(await misfit.allowlist(addr1.address)).to.equal(ethers.BigNumber.from(0))
        expect(await misfit.allowlist(addr2.address)).to.equal(ethers.BigNumber.from(0))
        const tx = await misfit.seedAllowlist(allowList,numSlots);
        // console.log("seedAllowList",tx)
        expect(await misfit.allowlist(addr1.address)).to.equal(ethers.BigNumber.from(3))
        expect(await misfit.allowlist(addr2.address)).to.equal(ethers.BigNumber.from(3))

        const allowList2 = [addr1.address,addr2.address];
        const numSlots2 = [5,0]
        const tx2 = await misfit.seedAllowlist(allowList2,numSlots2);
        // console.log("seedAllowList2",tx2)
        expect(await misfit.allowlist(addr1.address)).to.equal(ethers.BigNumber.from(5))
        expect(await misfit.allowlist(addr2.address)).to.equal(ethers.BigNumber.from(0))

    })

    it.only("other users cannot access seedAllowList", async function () {
        const allowList = [addr1.address,addr2.address];
        const numSlots = [3,3]
        await expect(
            addr1Misfit.seedAllowlist(allowList,numSlots)
        ).to.be.revertedWith("Ownable: caller is not the owner");

    })
    it.only("users not on allowList cannot buy", async function () {

        await expect(
            addr1Misfit.allowlistMint(1)
        ).to.be.revertedWith("not eligible for allowlist mint");

    })
    it.only("users on allowList can buy up to allowed qty", async function () {
        const allowList = [addr1.address,addr2.address];
        const numSlots = [3,3]
        const tx = await misfit.seedAllowlist(allowList,numSlots);

        await expect(
            addr1Misfit.allowlistMint(4,{value: minPrice.mul(4)})
        ).to.be.revertedWith("you are not eligible for so many");

        await batchMint(addr1Misfit,addr1.address,3)
    })

    it.only("users on allowList can only use voucher once", async function () {
        const allowList = [addr1.address,addr2.address];
        const numSlots = [3,3]
        const tx = await misfit.seedAllowlist(allowList,numSlots);


        await batchMint(addr1Misfit,addr1.address,0)
        await expect(
            addr1Misfit.allowlistMint(1,{value: minPrice.mul(1)})
        ).to.be.revertedWith("not eligible for allowlist mint");
    })
